...
import ExamModal from "./components/Modal/index.js";
...
export default function View() {
  ...
  const fetchData = async (search) => {
    ...
    try {
      let respond = await examCategoryApi.fetchData({ skip: currentPage, limit: rowPerPage, search: search});
  ...
  const handleEdit = (isDelete, event) => {
    if (isDelete) {
      try {
        examCategoryApi.delete(event._id);
